...
ProductCategory varchar(25) not null constraint PCategoryIN check(ProductCategory in('Camping Equipment','Outdoor Equipment','Personal Accessories','Mountaineering Equipment','Golf Equipment')),
ProductColor varchar(10) not null,
ProductSize varchar(5) not null,
ProductMaterial varchar(15) not null,
...
--created by Arshpreet on 1/10/2019
--Procedure for adding the product to the Product table
alter procedure AddProduct (@ProductID varchar(30), @ProductName varchar(15), @ProductNumber int, 
@ProductCategory varchar(25), @ProductColor varchar(10), @ProductSize varchar(5), @ProductMaterial varchar(15), 
@ProductPrice money )
as
BEGIN
begin try
...
insert into Product values(@ProductID,@ProductName,@ProductNumber,@ProductCategory,@ProductColor,@ProductSize,@ProductMaterial,@ProductPrice
...
exec AddProduct 'A1002','rope',3,'Outdoor Equipment','green','M','nylon',11500
exec AddProduct 'A1003','tent',4,'Camping Equipment','green','XL','rayon',13500
exec AddProduct 'A1004','rope',5,'Outdoor Equipment','green','s','nylon',8500

select * from Product

delete from Product

--created by Arshpreet on 3/10/2019
--Procedure to update the product price in the Product table where ProductID is a match
alter procedure UpdateProductPrice (@ProductID varchar(30), @ProductName varchar(15), @ProductNumber int, 
@ProductCategory varchar(25), @ProductColor varchar(10), @ProductSize varchar(5), @ProductMaterial varchar(15), 
@ProductPrice money)
as 
BEGIN
...
alter procedure UpdateProductDescription (@ProductID varchar(30), @ProductName varchar(15), @ProductNumber int, 
@ProductCategory varchar(25), @ProductColor varchar(10), @ProductSize varchar(5), @ProductMaterial varchar(15), 
@ProductPrice money)
as 
BEGIN
begin try
if exists(select ProductID from Product where ProductID = @ProductID) 
update Product set  ProductColor = @ProductColor,ProductSize = @ProductSize,ProductMaterial = @ProductMaterial,LastModifiedDateTime=sysdatetime() where ProductID = @ProductID;
else
throw 500002, 'Product does not exist',1 
end try
begin catch
...
alter procedure GetProductByProductID(@ProductID varchar(30))
as 
BEGIN
begin try
	if exists(select ProductID from Product where ProductID = @ProductID) 
...
alter procedure GetProductsByProductCategory(@ProductCategory varchar(25))
as 
BEGIN
begin try
...
alter procedure GetProductsByProductName( @ProductName varchar(15))
as 
BEGIN
begin try
...
alter procedure DeleteProduct(@ProductID varchar(30))
as 
BEGIN
begin try
	if exists(select ProductID from Product where ProductID = @ProductID) 
